...
def ious(atlbrs, btlbrs):
    """
    Compute IoU matrix between atlbrs and btlbrs.
    """
    if len(atlbrs) == 0 or len(btlbrs) == 0:
        return np.zeros((len(atlbrs), len(btlbrs)), dtype=np.float32)

    # Ensure both are float32 and C-contiguous arrays
    atlbrs = np.ascontiguousarray(np.asarray(atlbrs, dtype=np.float32))
    btlbrs = np.ascontiguousarray(np.asarray(btlbrs, dtype=np.float32))

    # print(atlbrs.dtype, atlbrs.flags['C_CONTIGUOUS'], atlbrs.shape)
    # print(btlbrs.dtype, btlbrs.flags['C_CONTIGUOUS'], btlbrs.shape)
...
#     ###
#     atlbrs = atlbrs.astype(np.float32)
#     btlbrs = btlbrs.astype(np.float32)
#     ###
